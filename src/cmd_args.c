...
static void process_parameter(char *param, struct main_parameters *main_params, struct gp_parameters *params)
{
    ...
    if (strncmp(key, "problem_dir", 11) == 0) {
        free(main_params->problem_dir);
        main_params->problem_dir = strdup(value);
    } else if (strncmp(key, "problem", 7) == 0) {
        main_params->problem_name = strdup(value);
    } else if (strncmp(key, "verbose", 7) == 0) {
        main_params->verbose = *value == 'Y' || *value == 'y';
    } else if (strncmp(key, "print_hold_out", 14) == 0) {
        main_params->print_hold_out = *value == 'Y' || *value == 'y';
    } else if (strncmp(key, "print_solution", 14) == 0) {
        main_params->print_solution = *value == 'Y' || *value == 'y';
    } else if (strncmp(key, "train_frac", 8) == 0) {
        main_params->train_frac = atof(value);
    } else if (strncmp(key, "rng_seed_file", 13) == 0) {
    ...
    } else if (strncmp(key, "lexicographic_tournament", 24) == 0) {
        params->lexicographic_tournament = *value == 'Y' || *value == 'y';
...
struct main_parameters *default_main_parameters()
{
    ...
    p->verbose = true;
    p->print_hold_out = false;
    p->print_solution = false;
